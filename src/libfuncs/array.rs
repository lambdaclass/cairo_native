...
use crate::{
    ...
    native_assert,
    types::array::{calc_metadata_size, metadata_data_ptr_offset, metadata_max_len_offset},
...
pub fn build_span_from_tuple<'ctx, 'this>(
    ...
    entry.store(context, location, metadata_ptr, k1)?;

    let max_len_ptr = entry.gep(
        context,
        location,
        metadata_ptr,
        &[GepIndex::Const(metadata_max_len_offset() as i32)],
    ...
    let data_ptr_field = entry.gep(
        ...
        location,
        metadata_ptr,
        &[GepIndex::Const(metadata_data_ptr_offset() as i32)],
        IntegerType::new(context, 8).into(),
    )?;
...
pub fn build_append<'ctx, 'this>(
    ...
    let array_obj = entry.append_op_result(scf::r#if(
        ...
        &[self_ty],
        {
            ...
            block.store(context, location, metadata_ptr, k1)?;
    ...
    // Update max length in metadata at offset 4
    let max_len_ptr = entry.gep(
        context,
        location,
        metadata_ptr,
