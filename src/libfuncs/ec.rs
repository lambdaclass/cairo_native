...
mod test {
    ...
    fn ec_neg() {
        ...
        assert_eq!(
            r(0.into(), 1.into()),
            [JITValue::EcPoint(0.into(), Felt252::from(-1))]
        );
        assert_eq!(
            r(1.into(), 0.into()),
        ...
        assert_eq!(
            r(1.into(), 1.into()),
            [JITValue::EcPoint(1.into(), Felt252::from(-1))]
        );
    }
    ...
    fn ec_point_from_x() {
        ...
        assert_eq!(r(1234.into()), [jit_enum!(0, JITValue::EcPoint(
            Felt252::from(1234),
            Felt252::from_dec_str("1301976514684871091717790968549291947487646995000837413367950573852273027507").unwrap()
        ))]);
    }
    ...
    fn ec_state_add() {
        run_program_assert_output(&EC_STATE_ADD, "run_test", &[
            JITValue::EcState(
                Felt252::from_dec_str("3151312365169595090315724863753927489909436624354740709748557281394568342450").unwrap(),
                Felt252::from_dec_str("2835232394579952276045648147338966184268723952674536708929458753792035266179").unwrap(),
                Felt252::from_dec_str("3151312365169595090315724863753927489909436624354740709748557281394568342450").unwrap(),
                Felt252::from_dec_str("2835232394579952276045648147338966184268723952674536708929458753792035266179").unwrap()
            ),
            JITValue::EcPoint(
                Felt252::from_dec_str("1234").unwrap(),
                Felt252::from_dec_str("1301976514684871091717790968549291947487646995000837413367950573852273027507").unwrap()
            )
        ], &[
            JITValue::EcState(
                Felt252::from_dec_str("763975897824944497806946001227010133599886598340174017198031710397718335159").unwrap(),
                Felt252::from_dec_str("2805180267536471620369715068237762638204710971142209985448115065526708105983").unwrap(),
                Felt252::from_dec_str("3151312365169595090315724863753927489909436624354740709748557281394568342450").unwrap(),
                Felt252::from_dec_str("2835232394579952276045648147338966184268723952674536708929458753792035266179").unwrap()
            )
        ]);
    ...
    fn ec_state_add_mul() {
        ...
        run_program_assert_output(&EC_STATE_ADD_MUL, "run_test", &[
            ...
            JITValue::EcState(
                Felt252::from_dec_str("3016674370847061744386893405108272070153695046160622325692702034987910716850").unwrap(),
                Felt252::from_dec_str("898133181809473419542838028331350248951548889944002871647069130998202992502").unwrap(),
                Felt252::from_dec_str("3151312365169595090315724863753927489909436624354740709748557281394568342450").unwrap(),
                Felt252::from_dec_str("2835232394579952276045648147338966184268723952674536708929458753792035266179").unwrap()
            )
        ]);
    ...
    fn ec_point_try_new_nz() {
        run_program_assert_output(
            ...
            "run_test",
            &[
                Felt252::from_dec_str("0").unwrap().into(),
                Felt252::from_dec_str("0").unwrap().into(),
            ],
            &[jit_enum!(1, jit_struct!())],
        );
        run_program_assert_output(
            ...
            "run_test",
            &[
                Felt252::from_dec_str("1234").unwrap().into(),
                Felt252::from_dec_str("1301976514684871091717790968549291947487646995000837413367950573852273027507").unwrap().into()
            ],
            &[
                jit_enum!(0, JITValue::EcPoint(
                    Felt252::from_dec_str("1234").unwrap(),
        ...
        run_program_assert_output(
            ...
            "run_test",
            &[  Felt252::from_dec_str("1234").unwrap().into(),
                Felt252::from_dec_str("1301976514684871091717790968549291947487646995000837413367950573852273027507").unwrap().neg().into()
                ],
            &[
                jit_enum!(0, JITValue::EcPoint(
                    Felt252::from_dec_str("1234").unwrap(),
                    Felt252::from_dec_str("1301976514684871091717790968549291947487646995000837413367950573852273027507").unwrap().neg()
                ))
            ],
        );
    }
    ...
    fn ec_point_unwrap() {
        fn parse(x: &str) -> Felt252 {
            if let Some(x) = x.strip_prefix('-') {
                Felt252::from_dec_str(x).unwrap().neg()
            } else {
                Felt252::from_dec_str(x).unwrap()
            }
        }
    ...
    fn ec_point_zero() {
        run_program_assert_output(
            ...
            &[],
            &[JITValue::EcPoint(
                Felt252::from_dec_str("0").unwrap(),
                Felt252::from_dec_str("0").unwrap().neg(),
            )],
        );
    }
}
