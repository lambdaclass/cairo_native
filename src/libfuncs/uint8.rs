...
pub fn build_square_root<'ctx, 'this>(
    ...
    let k1 = entry.const_int_from_type(context, location, 1, i8_ty)?;
    ...
    let result = entry.append_op_result(scf::r#if(
        ...
        },
        {
            ...
            let k8 = entry.const_int_from_type(context, location, 8, i8_ty)?;
            ...
            let num_bits = block.append_op_result(arith::subi(k8, leading_zeros, location))?;

            let shift_amount = block.append_op_result(arith::addi(num_bits, k1, location))?;

            let parity_mask = block.const_int_from_type(context, location, -2, i8_ty)?;
            let shift_amount =
                block.append_op_result(arith::andi(shift_amount, parity_mask, location))?;

            let k0 = block.const_int_from_type(context, location, 0, i8_ty)?;
            let result = block.append_op_result(scf::r#while(
                ...
                &[i8_ty, i8_ty],
                {
                    let region = Region::new();
                    let block =
                        region.append_block(Block::new(&[(i8_ty, location), (i8_ty, location)]));
                    ...
                    let threshold_is_poison =
                        block.cmpi(context, CmpiPredicate::Eq, block.arg(1)?, k8, location)?;
                    ...
                    let k2 = block.const_int_from_type(context, location, 2, i8_ty)?;
...
pub fn build_from_felt252<'ctx, 'this>(
    ...
    let const_max = entry.const_int_from_type(context, location, u8::MAX, felt252_ty)?;
...
mod test {
    ...
    fn u8_from_felt252() {
        ...
        run_program_assert_output(
            ...
            "run_test",
            &[],
            jit_struct!(jit_enum!(0, 255u8.into()), jit_enum!(1, jit_struct!()),),
        );
    }
    ...
    fn u8_sqrt() {
        ...
        for i in 0..u8::BITS {
            ...
            let y: u8 = BigUint::from(x)
                .sqrt()
                .try_into()
                .expect("should always fit into a u8");
    ...
    fn u8_widemul() {
        ...
        run_program_assert_output(program, "run_test", &[0u8.into(), 0u8.into()], 0u16.into());
        run_program_assert_output(program, "run_test", &[0u8.into(), 1u8.into()], 0u16.into());
        run_program_assert_output(program, "run_test", &[1u8.into(), 0u8.into()], 0u16.into());
        run_program_assert_output(program, "run_test", &[1u8.into(), 1u8.into()], 1u16.into());
        run_program_assert_output(
            program,
            "run_test",
