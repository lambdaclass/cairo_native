...
pub fn build_equal<'ctx, 'this>(
    ...
    let op0 = entry.append_op_result(arith::cmpi(
        context,
        CmpiPredicate::Eq,
        arg0,
        arg1,
    ...
    entry.append_operation(helper.cond_br(context, op0, [1, 0], [&[]; 2], location));
...
pub fn build_from_felt252<'ctx, 'this>(
    ...
    let const_max = entry.const_int_from_type(context, location, i8::MAX, felt252_ty)?;

    let const_min = entry.const_int_from_type(context, location, i8::MIN, felt252_ty)?;

    let mut block = entry;

    // make unsigned felt into signed felt
    // felt > half prime = negative
    let halfprime_i252 = metadata
    ...
    let value = {
        let half_prime: melior::ir::Value =
            block.const_int_from_type(context, location, halfprime_i252, felt252_ty)?;
...
mod test {
    ...
    fn i8_from_felt252() {
        ...
        run_program_assert_output(
            ...
            "run_test",
            &[],
            jit_struct!(jit_enum!(0, 127i8.into()), jit_enum!(1, jit_struct!()),),
        );
    }
    ...
    fn i8_widemul() {
        ...
        run_program_assert_output(program, "run_test", &[0i8.into(), 0i8.into()], 0i16.into());
        run_program_assert_output(program, "run_test", &[0i8.into(), 1i8.into()], 0i16.into());
        run_program_assert_output(program, "run_test", &[1i8.into(), 0i8.into()], 0i16.into());
        run_program_assert_output(program, "run_test", &[1i8.into(), 1i8.into()], 1i16.into());
        run_program_assert_output(
            program,
            "run_test",
            &[i8::MAX.into(), i8::MAX.into()],
