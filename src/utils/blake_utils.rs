// This file is heavily inspired in blake2s hash compression implementation from LambdaClass's cairo-vm
// For more info check: https://github.com/lambdaclass/cairo-vm/blob/main/vm/src/hint_processor/builtin_hint_processor/blake2s_hash.rs
...
pub fn blake2s_compress<'a>(
    state: &'a[u32; 8], // current state
    message: &'a[u32; 16], // message to compress
    // next four params are the count of bytes that had been fed to the compression
    // this count is a u128 separeted into four
    t0: u32,
    t1: u32,
    f0: u32,
    f1: u32,
) -> [u32; 8] {
    let mut vector = state.to_vec();
    vector.extend(&IV[0..4]);
    vector.extend(&vec![
        (IV[4] ^ t0),
        (IV[5] ^ t1),
        (IV[6] ^ f0),
    ...
    for sigma_list in SIGMA {
        vector = blake_round(vector, message, sigma_list);
    }
    let mut new_state: [u32; 8] = [0; 8];
    for i in 0..8 {
        new_state[i] = state[i] ^ vector[i] ^ vector[8 + i];
    }
