...
{
    let ptr = arena
        .alloc_layout(get_integer_layout(252).repeat(4).unwrap().0)
        .cast();

    let data = <[[u32; 8]; 4] as Deserialize>::deserialize(deserializer)?;
    assert!(BigUint::new(data[0].to_vec()) < *PRIME);
    assert!(BigUint::new(data[1].to_vec()) < *PRIME);
    assert!(BigUint::new(data[2].to_vec()) < *PRIME);
    assert!(BigUint::new(data[3].to_vec()) < *PRIME);
