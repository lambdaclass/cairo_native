        ...
        |statement_idx, (mut state, mut tailrec_state)| {
            ...
            Ok(match &statements[statement_idx.0] {
                Statement::Invocation(invocation) => {
                    ...
                    if let Some(trace_meta) =
                        metadata.get_mut::<crate::metadata::trace_dump::TraceDump>()
                    {
                        trace_dump::build_state_snapshot(
                            trace_meta,
                            context,
                            registry,
                            module,
...
#[cfg(feature = "with-trace-dump")]
mod trace_dump {
    // Note: This module only works with the JIT engine. It does NOT work with AOT.
    ...
    /// Make a snapshot (not Sierra snapshot) of the Sierra state and push it.
    #[allow(clippy::too_many_arguments)]
    pub fn build_state_snapshot(
        trace_meta: &mut TraceDump,
        context: &Context,
        registry: &ProgramRegistry<CoreType, CoreLibfunc>,
        module: &Module,
        init_block: &BlockRef,
