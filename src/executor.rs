            ...
            _ if type_info.is_builtin() => {
                if !type_info.is_zst(registry)? {
                    if let CoreTypeConcrete::BuiltinCosts(_) = type_info {
                        // todo: should we use this value? See: https://github.com/starkware-libs/cairo_native/issues/1219
...
pub(crate) struct InvocationGuard {
    ...
    // TODO: Remove this field — cleanup can be done in dict destructs instead.
...
mod tests {
    ...
    use crate::{
        ...
        utils::testing::{load_contract, load_program},
        OptLevel,
    };
    ...
    fn starknet_program() -> Program {
        load_contract("contracts/simple_storage_42.contract.json")
