...
run_bench() {
    ...
    CAIRO_NATIVE_RUNTIME_LIBDIR="$ROOT_DIR/target/release" hyperfine \
        ...
        -n "cairo-native (embedded AOT)" "$JIT_CLI --run-mode=aot $base_path.cairo --opt-level 3" \
        -n "cairo-native (embedded JIT using LLVM's ORC Engine)" "$JIT_CLI --run-mode=jit $base_path.cairo --opt-level 3" \
