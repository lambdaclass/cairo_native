...
use cairo_native::{executor::AotContractExecutor, starknet_stub::StubSyscallHandler};
...
fn keccak_test() {
    ...
    assert_eq!(
        ...
        340282366920938463463374607431768143515
    );
    assert_eq!(result.return_values, vec![1.into()]);
}

#[test]
fn keccak_test_contract_executor() {
    let contract = &KECCAK_CONTRACT;

    let entry_point = contract.entry_points_by_type.external.first().unwrap();

    let program = contract.extract_sierra_program().unwrap();
    let executor = AotContractExecutor::new(&program, cairo_native::OptLevel::Default).unwrap();
    let result = executor
        .run(
            &FunctionId::new(entry_point.function_idx as u64),
            &[],
            u128::MAX.into(),
            &mut StubSyscallHandler::default(),
        )
