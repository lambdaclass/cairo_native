...
impl<'ctx> Compiler<'ctx> {
    ...
    pub fn create_libfunc_struct_construct(
...

        {
            ...
            storage
                .functions
                .insert(id, FunctionDef { args: arg_types, return_types: vec![return_type] });
        }
    ...
    pub fn create_libfunc_store_temp(
        ...
        let arg_types = arg_types_with_locations.iter().map(|(t, _)| *t).collect_vec();
...

        {
            ...
            storage
                .functions
                .insert(id, FunctionDef { args: arg_types.clone(), return_types: arg_types });
        }
    ...
    pub fn create_libfunc_dup(
        ...
        let arg_types = arg_types_with_locations.iter().map(|(t, _)| *t).collect::<Vec<_>>();
        ...
        {
            let mut storage = storage.borrow_mut();
            storage.functions.insert(id, FunctionDef { args: arg_types, return_types });
        }
    ...
    pub fn create_libfunc_felt_add(
        ...
        storage.borrow_mut().functions.insert(
            id,
            FunctionDef { args: vec![felt_type, felt_type], return_types: vec![felt_type] },
        );
