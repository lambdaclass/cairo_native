use integer::{i32_overflowing_add_impl, i32_overflowing_sub_impl, SignedIntegerResult};
...
fn main() -> (
    ...
    // Underflow
    let (res_c, flag_c) = overflowing_add(-2000000000, -2000000000);
    // Overflow
    let (res_d, flag_d) = overflowing_add(2000000000, 2000000000);

    // In range subtractions
    let (res_e, flag_e) = overflowing_sub(16, 1);
    let (res_f, flag_f) = overflowing_sub(1, 16);
    // Underflow
    let (res_g, flag_g) = overflowing_sub(-2000000000, 2000000000);
    // Overflow
    let (res_h, flag_h) = overflowing_sub(2000000000, -2000000000);
