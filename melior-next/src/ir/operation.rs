...
impl<'c> Operation<'c> {
    ...
    pub fn region(&self, index: usize) -> Option<RegionRef> {
        unsafe {
            if index < self.region_count() {
                Some(RegionRef::from_raw(mlirOperationGetRegion(self.raw, index as isize)))
            } else {
                None
            }
        }
    ...
    pub(crate) unsafe fn from_raw(raw: MlirOperation) -> Self {
        Self { raw, _context: Default::default() }
    }
...
impl<'a> OperationRef<'a> {
    ...
    pub(crate) unsafe fn from_raw(raw: MlirOperation) -> Self {
        Self { raw, _reference: Default::default() }
    }
...
mod tests {
    ...
    fn name() {
        ...
        assert_eq!(
            Builder::new("foo", Location::unknown(&context)).build().name(),
    ...
    fn block_none() {
        let context = Context::new();
        context.set_allow_unregistered_dialects(true);
        assert_eq!(Builder::new("foo", Location::unknown(&context)).build().block(), None);
    }
    ...
    fn result_error() {
        ...
        context.set_allow_unregistered_dialects(true);
        assert_eq!(
            Builder::new("foo", Location::unknown(&context)).build().result(0).unwrap_err(),
    ...
    fn region_none() {
        let context = Context::new();
        context.set_allow_unregistered_dialects(true);
        assert!(Builder::new("foo", Location::unknown(&context),).build().region(0).is_none());
    }
    ...
    fn display() {
        ...
        assert_eq!(
            Builder::new("foo", Location::unknown(&context),).build().to_string(),
            "\"foo\"() : () -> ()\n"
        );
    }
    ...
    fn debug() {
        ...
        assert_eq!(
            format!("{:?}", Builder::new("foo", Location::unknown(&context)).build()),
            "Operation(\n\"foo\"() : () -> ()\n)"
        );
    }
