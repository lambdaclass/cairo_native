...
impl<'c> Block<'c> {
    ...
    pub fn argument(&self, position: usize) -> Result<Argument, Error> {
        unsafe {
            if position < self.argument_count() {
                Ok(Argument::from_raw(mlirBlockGetArgument(self.raw, position as isize)))
            } else {
    ...
    pub fn add_argument(&self, r#type: Type<'c>, location: Location<'c>) -> Value {
        unsafe {
            Value::from_raw(mlirBlockAddArgument(self.raw, r#type.to_raw(), location.to_raw()))
        }
    }
...
mod tests {
    ...
    fn argument() {
        ...
        assert_eq!(
            Block::new(&[(r#type, Location::unknown(&context))]).argument(0).unwrap().r#type(),
            r#type
        );
    }
