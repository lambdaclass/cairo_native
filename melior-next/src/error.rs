    ...
    fn fmt(&self, formatter: &mut Formatter) -> fmt::Result {
        match self {
            ...
            Self::BlockArgumentPosition(block, position) => {
                write!(formatter, "block argument position {position} out of range: {block}")
            }
            Self::FunctionExpected(r#type) => write!(formatter, "function expected: {type}"),
            Self::FunctionInputPosition(r#type, position) => {
                write!(formatter, "function input position {position} out of range: {type}")
            }
            Self::FunctionResultPosition(r#type, position) => {
                write!(formatter, "function result position {position} out of range: {type}")
            }
            ...
            Self::OperationResultPosition(operation, position) => {
                write!(formatter, "operation result position {position} out of range: {operation}")
            }
            ...
            Self::TupleFieldPosition(r#type, position) => {
                write!(formatter, "tuple field position {position} out of range: {type}")
            }
