...
build-dev: check-llvm
	cargo +nightly build --profile optimized-dev --features=scarb
...
.PHONY: runtime-ci
runtime-ci:
	cargo +nightly b --profile ci --all-features -p cairo-native-runtime && cp target/ci/libcairo_native_runtime.a .
